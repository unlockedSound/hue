  ...
  function multi(xAxis) {
    ...
    return d3v3.time.format.utc.multi([
      ["%S %Y-%m-%dT%H:%M", function(d) {
        ...
        var isFirstOrLast = d == domain[0] || d == domain[domain.length - 1];
        var result = isFirstOrLast && domainDiff < MINUTE_MS * minDiff;
        if (result) {
          previous = d;
        }
        return result;
      ...
      ["%H:%M %Y-%m-%d", function(d) {
        ...
        var domainDiff = Math.abs(domain[domain.length - 1] - domain[0]);
        var isFirstOrLast = d == domain[0] || d == domain[domain.length - 1];
        var result = isFirstOrLast && domainDiff < HOUR_MS * minDiff;
        if (result) {
          previous = d;
        }
        return result;
      ...
      ["%H:%M %Y-%m-%d", function(d) {
        ...
        var domainDiff = Math.abs(domain[domain.length - 1] - domain[0]);
        var isFirstOrLast = d == domain[0] || d == domain[domain.length - 1];
        var result = isFirstOrLast && domainDiff < DAY_MS * minDiff;
        if (result) {
          previous = d;
        }
        return result;
      ...
      ["%d %Y-%m", function(d) {
        ...
        var domainDiff = Math.abs(domain[domain.length - 1] - domain[0]);
        var isFirstOrLast = d == domain[0] || d == domain[domain.length - 1];
        var result = isFirstOrLast && domainDiff < MONTH_MS * minDiff;
        if (result) {
          previous = d;
        }
        return result;
      ...
      ["%m %Y", function(d) {
        ...
        var domainDiff = Math.abs(domain[domain.length - 1] - domain[0]);
        var isFirstOrLast = d == domain[0] || d == domain[domain.length - 1];
        var result = isFirstOrLast && domainDiff < YEAR_MS * minDiff;
        if (result) {
          previous = d;
        }
        return result;
